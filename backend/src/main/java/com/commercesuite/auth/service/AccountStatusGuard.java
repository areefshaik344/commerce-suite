...
public class AccountStatusGuard {
    ...
    public void assertCanLogin(User u) {
        ...
        if (!u.getAccountStatus().canLogin())
            throw AppException.unauthorized(ErrorCode.ACCOUNT_NOT_ACTIONABLE, "Account " + u.getAccountStatus());
        if (u.getLockedUntil() != null && u.getLockedUntil().isAfter(Instant.now(clock)))
