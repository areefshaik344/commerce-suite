...
public class RefreshTokenService {
    ...
    /** Rotate: validate -> revoke -> issue new in same family. Reuse detected => revoke whole family. */
    @Transactional
    public IssuedRefresh rotate(String rawPresented, String userAgent, String ip) {
        ...
        if (current.getRevokedAt() != null) {
            // Reuse attempt — revoke whole family for safety.
    ...
    public void revoke(String rawPresented) {
        repo.findByTokenHash(HashUtil.sha256(rawPresented))
                .ifPresent(rt -> { if (rt.getRevokedAt() == null) rt.setRevokedAt(Instant.now()); });
    }
