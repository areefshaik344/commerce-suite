...
/**
 * Child (Vendor) Order FSM — authoritative spec: docs/ORDER_FSM.md §2.
 *
 * Initial state per spec is `PENDING_PAYMENT`; `CREATED` retained as a
 * back-compat alias until Phase 7 payments rewires creation.
 * `COMPLETED` replaces the prior `CLOSED` terminal (alias retained).
 */
public enum VendorOrderStatus {
  PENDING_PAYMENT, CREATED, CONFIRMED, PROCESSING, PACKED, SHIPPED, OUT_FOR_DELIVERY,
  DELIVERED, CANCELLED, RETURN_REQUESTED, RETURNED, REFUNDED, COMPLETED, CLOSED;
  public boolean isTerminal() {
    return this == CANCELLED || this == REFUNDED || this == COMPLETED || this == CLOSED;
  }
  public boolean isCancellable() {
    return this == PENDING_PAYMENT || this == CREATED
        || this == CONFIRMED || this == PROCESSING || this == PACKED;
  }
  ...
  private static final Map<VendorOrderStatus,Set<VendorOrderStatus>> ALLOWED = Map.ofEntries(
    ...
    Map.entry(DELIVERED, Set.of(RETURN_REQUESTED, COMPLETED, CLOSED)),
    Map.entry(RETURN_REQUESTED, Set.of(RETURNED, DELIVERED)),
    Map.entry(RETURNED, Set.of(REFUNDED)),
    Map.entry(REFUNDED, Set.of(COMPLETED, CLOSED)),
